...
static int GM_CDECL gmThreadIsAlive( gmThread * a_thread )
{
	GM_CHECK_NUM_PARAMS( 1 );
	...
	a_thread->PushInt( a_thread->GetMachine()->GetThread( id ) != 0 );
	return GM_OK;
}
...
static int GM_CDECL gmThreadGroup(gmThread * a_thread) // return thread group
{
	GM_CHECK_NUM_PARAMS( 0 );
	a_thread->PushInt(a_thread->GetGroup());
	return GM_OK;  
}
...
// Callback iteration function for gmKillAllThreads() 
// Kills all threads except current, current thread is passed in as context.
static bool threadIterKillGroup(gmThread * a_thread, void * a_context)
{
  int group = *(int*)a_context;
  
  if( a_thread->GetGroup() == group ) // Ignore calling thread
  {
    switch(a_thread->GetState())
    {
      case gmThread::RUNNING:
...
static int GM_CDECL gmKillThreadsInGroup(gmThread * a_thread) // thread id
{
  GM_CHECK_INT_PARAM(group, 0);

  GM_ASSERT( group != 0 ); // cannot kill main thread!

  a_thread->GetMachine()->ForEachThread(threadIterKillGroup, &group);
  return (a_thread->GetGroup() == group) ? GM_SYS_KILL : GM_OK;
}


static int GM_CDECL gmSetThreadGroup(gmThread * a_thread) // thread id
{
  GM_CHECK_INT_PARAM(group, 0);
  a_thread->SetGroup(group);
  return GM_OK;
}
...
static int GM_CDECL gmTableClear(gmThread * a_thread)
{
  GM_CHECK_NUM_PARAMS(1);
  GM_CHECK_TABLE_PARAM(table, 0);
  table->ClearTable(a_thread->GetMachine(), false);
  return GM_OK;
}
...
{ 
  ...
  */
  ...
  /*gm
    ...
    \brief returns true if the thread id is valid and alive
    \return table of thread Ids
  */
  {"threadIsAlive", gmThreadIsAlive},
  /*gm
    \function threadId
  ...
  */
  ...
  \brief threadGroup will return the group of current thread
  \return int 
  */
  {"threadGroup", gmThreadGroup},
  /*gm
    \function threadKillGroup
    \brief threadKill will kill all threads in group
  */
  {"threadKillGroup", gmKillThreadsInGroup},
  /*gm
    \function threadSetGroup
    \brief threadKillAll will kill all the threads except the current one
    \param int group
  */
  {"threadSetGroup", gmSetThreadGroup},
  ...
  */
  ...
  /*gm
    \function tableClear
    \brief tableCount will clear the members of the table
    \param table
  */
