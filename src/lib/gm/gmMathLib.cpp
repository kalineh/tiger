...
{
	GM_CHECK_NUM_PARAMS(2);
	GM_CHECK_VEC3_PARAM(vec,		0);
	GM_CHECK_FLOAT_PARAM(angleRad,	1);

	a_thread->PushVec3( rotateAxisX(vec,angleRad) );
	return GM_OK;
}
...
static int GM_CDECL gmfRotateAxisY(gmThread * a_thread)
{
	...
	a_thread->PushVec3( rotateAxisY(vec,angleRad) );
	return GM_OK;
}
...
static int GM_CDECL gmfRotateAxis(gmThread * a_thread)
{
	...
	GM_CHECK_VEC3_PARAM(vec,		0);
	GM_CHECK_FLOAT_PARAM(angleRad,	1);
	GM_CHECK_VEC3_PARAM(axis,		2);

	a_thread->PushVec3( rotateAxis(vec, angleRad, axis) );
	return GM_OK;
}
