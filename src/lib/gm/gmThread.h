...
public:
  ...
  inline void PushVec2(funk::v2 a_value);
  inline void PushVec3(funk::v3 a_value);
  ...
  inline gmVec2 ParamVec2(int a_param) const;
  inline bool ParamVec2(int a_param, funk::v2& a_value) const;
  inline gmVec3 ParamVec3(int a_param) const;
  inline bool ParamVec3(int a_param, funk::v3& a_value) const;
...
private:
  ...
  // EDDIE
  State gmThread::Sys_ExecuteMainLoop(gmVariable * a_return);
...
inline void gmThread::PushVec2(funk::v2 a_value)
{
	m_stack[m_top].m_type = GM_VEC2;
	m_stack[m_top++].m_value.m_v2 = a_value.gmv2;
}

inline void gmThread::PushVec3(funk::v3 a_value)
{
...
inline gmVec2 gmThread::ParamVec2(int a_param) const
{
	const gmVec2 default_val = {0,0};

	if(a_param >= m_numParameters) return default_val;
	gmVariable * var = m_stack + m_base + a_param;
	if(var->m_type == GM_VEC2) return var->m_value.m_v2;
	return default_val;
}

inline bool gmThread::ParamVec2(int a_param, funk::v2& a_value) const
{
	const gmVec2 default_val = {0,0};
...
inline gmVec3 gmThread::ParamVec3(int a_param) const
{
	const gmVec3 default_val = {0,0,0};

	if(a_param >= m_numParameters) return default_val;
	gmVariable * var = m_stack + m_base + a_param;
	if(var->m_type == GM_VEC3) return var->m_value.m_v3;
	return default_val;
}
inline bool gmThread::ParamVec3(int a_param, funk::v3& a_value) const
{
	const gmVec3 default_val = {0,0,0};
...
#define GM_NUM_PARAMS GM_THREAD_ARG->GetNumParams()
  ...
  #define GM_VEC2_PARAM(VAR, PARAM) funk::v2 VAR; if( !GM_THREAD_ARG->ParamVec2((PARAM), (VAR)) )  { return GM_EXCEPTION; }
  #define GM_VEC3_PARAM(VAR, PARAM) funk::v3 VAR; if( !GM_THREAD_ARG->ParamVec3((PARAM), (VAR)) )  { return GM_EXCEPTION; }
...
#define GM_OP_USER_PTR( TYPE, INDEX, PTR ) GM_ASSERT( a_operands[INDEX].m_type == GM_TYPEID(TYPE) ); TYPE* PTR = static_cast<TYPE*>(static_cast< gmUserObject*>(GM_OBJECT(a_operands[INDEX].m_value.m_ref))->m_user);
#define GM_OP_STR_PTR( CHAR_PTR, INDEX) GM_ASSERT( a_operands[INDEX].m_type == GM_STRING ); const char* CHAR_PTR = (static_cast< gmStringObject*>(GM_OBJECT(a_operands[INDEX].m_value.m_ref)))->GetString();
#define GM_OP_VEC2( VAL, INDEX ) GM_ASSERT( a_operands[INDEX].m_type == GM_VEC2 ); gmVec2 &VAL = a_operands[INDEX].m_value.m_v2;
#define GM_OP_VEC3( VAL, INDEX ) GM_ASSERT( a_operands[INDEX].m_type == GM_VEC3 ); gmVec3 &VAL = a_operands[INDEX].m_value.m_v3;
#define GM_OP_FLOAT( VAL, INDEX) GM_ASSERT( a_operands[INDEX].m_type == GM_FLOAT ); float VAL = a_operands[INDEX].m_value.m_float;
#define GM_OP_INT( VAL, INDEX) GM_ASSERT( a_operands[INDEX].m_type == GM_INT ); int VAL = a_operands[INDEX].m_value.m_int;
#define GM_OP_FLOAT_OR_INT( VAL, INDEX ) GM_ASSERT( a_operands[INDEX].m_type == GM_FLOAT || a_operands[INDEX].m_type == GM_INT ); float VAL = (float)(a_operands[INDEX].m_type == GM_INT )?a_operands[INDEX].m_value.m_int:a_operands[INDEX].m_value.m_float;
...
#define GM_CHECK_VEC2_PARAM(VAR, PARAM) \
	if(GM_THREAD_ARG->ParamType((PARAM)) != GM_VEC2) { GM_EXCEPTION_MSG("expecting param %d as vec2", (PARAM)); return GM_EXCEPTION; } \
