...
struct gmfGfxLib
{
	...
	static int GM_CDECL gmfTexCoord(gmThread * a_thread)
	{
		...
		else if ( numParams == 2 )
		{
			...
			GM_CHECK_FLOAT_OR_INT_PARAM( v, 1 );
	...
	static int GM_CDECL gmfDrawArcDonut(gmThread * a_thread)
	{
		GM_CHECK_VEC2_PARAM( center, 0 );
		GM_CHECK_FLOAT_PARAM( radius, 1);
		GM_CHECK_FLOAT_PARAM( thickness, 2);
		...
		DrawArcDonut( center, radius, thickness, startAngle, endAngle, numSegs);

		return GM_OK;
	}

	static int GM_CDECL gmfDrawArcLine(gmThread * a_thread)
	{
		GM_CHECK_VEC2_PARAM( center, 0 );
		GM_CHECK_FLOAT_PARAM( radius, 1);
		...
		DrawArcLine( center, radius, startAngle, endAngle, numSegs);
	...
	static int GM_CDECL gmfDrawBasisVectors(gmThread * a_thread)
	{
		GM_CHECK_NUM_PARAMS(0);
		DrawBasisVectors();
		return GM_OK;
	...
	static int GM_CDECL gmfDrawArrow(gmThread * a_thread)
	{
		GM_CHECK_NUM_PARAMS(2);
		GM_CHECK_VEC3_PARAM( start, 0 );
		GM_CHECK_VEC3_PARAM( end, 1 );

		DrawArrow( start, end );
		return GM_OK;
	}

	static int GM_CDECL gmfDrawDisk(gmThread * a_thread)
	{
		GM_CHECK_VEC3_PARAM( center, 0 );
		GM_CHECK_VEC3_PARAM( dir, 1 );
		GM_CHECK_FLOAT_OR_INT_PARAM( radius, 2 );
		GM_INT_PARAM( numSteps, 3, 16 );

		DrawDisk( center, dir, radius, numSteps );
		return GM_OK;
	}

	static int GM_CDECL gmfDrawDiskWire(gmThread * a_thread)
	{
		GM_CHECK_VEC3_PARAM( center, 0 );
		GM_CHECK_VEC3_PARAM( dir, 1 );
		GM_CHECK_FLOAT_OR_INT_PARAM( radius, 2 );
		...
		DrawDiskWire( center, dir, radius, numSteps );
		return GM_OK;
	}
...
static gmFunctionEntry s_gmGfxLib[] = 
{
	...
	GM_LIBFUNC_ENTRY(DrawArcDonut, Gfx)
	GM_LIBFUNC_ENTRY(DrawArcLine, Gfx)
	...
	GM_LIBFUNC_ENTRY(DrawBasisVectors, Gfx)
	GM_LIBFUNC_ENTRY(DrawArrow, Gfx)
	GM_LIBFUNC_ENTRY(DrawDisk, Gfx)
	GM_LIBFUNC_ENTRY(DrawDiskWire, Gfx)
